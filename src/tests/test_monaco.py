...
def test_typical_build_report(drivers_data):
    with mock.patch('builtins.open') as patched_open:
        ...
        assert monaco.build_report("data") == drivers_data["report"]


def test_argparse():
    a = ["--file", "data", "driver", "Sebastian Vettel"]
    assert monaco.input_from_argparse(a) == argparse.Namespace(asc=False,
                                                               desc=False,
